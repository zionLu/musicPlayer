...
function change(){//交替大周期
	...
	changeCurcit = setTimeout("change()",30000);
}
...
function setAudioAPI(){
	try{
		...
		source = context.createMediaElementSource(audio);//设源
		analyser = context.createAnalyser();//分析器
		//流......连接：source → analyser → context.destination
		source.connect(analyser);
		analyser.connect(context.destination);
		//缓冲区长度(这里......最懵逼......)
		var length = analyser.frequencyBinCount;
		//创建数组以接收数据(据说是快速傅立叶变换的结果；一个长度为940的unit8array这个数组的平均数表征了音频的“噪度”，以数组下标为x轴，数组值为y轴做出的图就是“频谱图”了)
